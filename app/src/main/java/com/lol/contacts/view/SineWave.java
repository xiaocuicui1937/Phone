...
public class SineWave extends View implements Runnable {
	...
	private static float phase = 45.0f; // ��λ
	...
	// �����д����Ĺ��캯������ᱨ��custom view SineWave is not using the 2- or 3-argument
	// View constructors
	...
	public void onDraw(Canvas canvas) {
...

		{
			...
			canvas.drawLine(
					...
					cy
							- amplifier
							* (float) (Math.sin(phase * 2 * (float) Math.PI
									/ 360.0f + 2 * Math.PI * frequency * i
									/ width)),
					(float) (i + 1),
					cy
							- amplifier
							* (float) (Math.sin(phase * 2 * (float) Math.PI
									/ 360.0f + 2 * Math.PI * frequency
									* (i + 1) / width)), mPaint);

			float point = cy
					- amplifier
					* (float) (Math.sin(phase * 2 * (float) Math.PI / 360.0f
							+ 2 * Math.PI * frequency * i / width));

			if ((py >= (point - 2.5f)) && (py <= (point + 2.5f))
					&& (px >= i - 2.5f) && (px <= i + 2.5f))
...

		{
			...
			canvas.drawText(
					"(x=" + Float.toString(px) + ",y=" + Float.toString(py)
							+ ")", 20, 20, mPaint);
		...
		canvas.drawText("(x=" + Float.toString(px) + ",y=" + Float.toString(py)
				+ ")", 20, this.getHeight() - 20, mPaint);
